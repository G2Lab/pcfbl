...
PATH = '/gpfs/commons/groups/gursoy_lab/aelhussein/patient_pfl/20_sites/single_task/'
...
def main(iteration):
    ...
    torch.save(initial_model.state_dict(), f'{PATH}{hospid}/prediction.pt')
    ...
    if auc<= 0.5:
        ...
        auc.to_csv(f'{PATH}{hospid}/results_{iteration}.csv', index = False)
        auprc.to_csv(f'{PATH}{hospid}/results_auprc_{iteration}.csv', index = False)
        return
